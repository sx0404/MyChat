...
func (self *OpenList) Push(x interface{}) {
	// Push and Pop use pointer receivers because they modify the slice's length,
	// not just its contents.
	*self = append(*self, x.(*_AstarPoint))
}

func (self *OpenList) Pop() interface{} {
	old := *self
	n := len(old)
	x := old[n-1]
	*self = old[0 : n-1]
...
func (this *Map) PrintMap(path *SearchRoad, doPrint bool) {
	if doPrint == false {
		return
	}
...
var FindPointError = errors.New("find target error")

func FindTargetPoint(strSlice []string, targetChar int32) (int, int, error) {
	for y, str := range strSlice {
	...
	return 0, 0, FindPointError
}

func AStar(DoPrint bool) {
	m := NewMap(aStartMap)
	m.PrintMap(nil, DoPrint)
	//if err != nil {
	//	fmt.Println("not find start point!!!!")
	//	return
	//}
	//fmt.Println("start point,x:", startX, ",y:", StartY)
	//endX, endY, err := FindTargetPoint(aStartMap, 'E')
	//if err != nil {
	//	fmt.Println("not find end point!!!!!")
	//	return
	//}
	...
	if searchRoad.FindoutRoad() {
		...
		fmt.Println("not find to！")
	}
}

//计算代码耗时
func CalcAStarTime(loopNum int) {
	defer UserTime()()
	for i := 0; i < loopNum; i++ {
		AStar(false)
	}
...
func UserTime() func() {
	BeginTime := time.Now().UnixNano() / 1e6 //毫秒
	return func() { fmt.Println("user time:", uint64(time.Now().UnixNano()/1e6)-uint64(BeginTime)) }
}
